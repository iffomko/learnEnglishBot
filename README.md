...
```
    ...
    8. Зарефакторить класс Context: нужно создавать параметры для пользователя в отдельном методе, переделать хранение данных, чтобы убрать
       четкую зависимость от строчек (уменьшает производительность)
```
